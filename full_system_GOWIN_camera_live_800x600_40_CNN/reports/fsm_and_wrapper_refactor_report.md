...
## Relaxed Acceptance Phase Note

The current relaxed structural candidate is documented in `reports/structural_candidate_refactor_report.md`. That newer report supersedes the strict rollback decisions in this file where they differ: `leaf_scalar_dense` is now kept, `leaf_rgb565_green_to_gray` is kept as a wrapper boundary, and the SPI three-block rewrite remains rolled back because its combined-candidate generated-clock TNS exceeded the relaxed 2x threshold.

## 2026-06-01 Overlay Phase Update

The later seven-segment overlay phase is documented in `reports/sevenseg_overlay_and_wrapper_refactor_report.md`. That phase restores a lightweight HDMI result digit, adds `cdc_result_class_latch`, `digit_to_7seg`, and `sevenseg_digit_overlay`, and keeps minimal MNIST threshold/address wrappers after re-testing. It does not change the FSM rollback decisions in this report.
