...
module streaming_mnist_capture #(
    ...
    output reg  [9:0]  mnist_write_count
);
...
localparam [18:0] ROI_PIXEL_TOTAL = ROI_SIZE * ROI_SIZE;
localparam [9:0]  MNIST_PIXEL_TOTAL = MNIST_SIZE * MNIST_SIZE;
...
wire [12:0] update_sum_next = update_sum_s1 + {4'd0, update_add_s1};
        ...
        case (state)
            ...
            S_WRITE_ROW: begin
                if (vsync_rise) begin
                    ...
                    mnist_write_count <= mnist_write_count + 10'd1;
                    ...
                    if (wr_col == (MNIST_SIZE - 1)) begin
                        ...
                        if (wr_row == (MNIST_SIZE - 1)) begin
                            capture_busy <= 1'b0;
                            state <= S_IDLE;
                            if ((roi_pixel_count == ROI_PIXEL_TOTAL) &&
                                (mnist_write_count == (MNIST_PIXEL_TOTAL - 1))) begin
                                capture_done <= 1'b1;
                            end else begin
                                capture_error <= 1'b1;
                            end
